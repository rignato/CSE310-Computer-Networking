...
import sys, time, datetime, socket, subprocess, os
...
def recursive_query(domain, ns):
    ...
    answers = []
    # if res.answer or not res.additional:
    #     res = query.udp(message.make_query(domain, rdatatype.A), ns)
    # print(res)
    if res.answer:
...
for d in domains:
    print(d)
    for i in range(10):
        # cmd = "dig %s @8.8.8.8 | grep -oP \"Query time: \\K.\"" % d
        # os.system(cmd)
        # print(get_time(d))
        # cmd = "dig %s | grep -oP \"Query time: \\K.\"" % d
        # os.system(cmd)
        cmd = "dig %s @8.8.8.8 | grep -oP \"Query time: \\K.\"" % d
        os.system(cmd)
